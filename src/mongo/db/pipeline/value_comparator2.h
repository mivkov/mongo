...
public:
    ...
    class EqualTo {
    public:
        explicit EqualTo(const Value2Comparator* comparator) : _comparator(comparator) {}
    ...
    private:
        const Value2Comparator* _comparator;
    };
    ...
    class LessThan {
    public:
        explicit LessThan(const Value2Comparator* comparator) : _comparator(comparator) {}
    ...
    class Hasher {
    public:
        explicit Hasher(const Value2Comparator* comparator) : _comparator(comparator) {}

        size_t operator()(const Value2& val) const {
    ...
    static const Value2Comparator kInstance;

    /**
     * Constructs a value comparator with simple comparison semantics.
     */
    Value2Comparator() = default;

    /**
     * Constructs a value comparator with special string comparison semantics.
     */
    Value2Comparator(const StringData::ComparatorInterface* stringComparator)
    ...
    int compare(const Value2& lhs, const Value2& rhs) const {
        return Value2::compare(lhs, rhs, _stringComparator);
    }
     ...
     */
    std::set<Value2, LessThan> makeOrderedValueSet() const {
     ...
     */
    stdx::unordered_set<Value2, Hasher, EqualTo> makeUnorderedValueSet() const {
        return stdx::unordered_set<Value2, Hasher, EqualTo>(0, Hasher(this), EqualTo(this));
    }
     ...
     */
    template <typename T>
    std::map<Value2, T, LessThan> makeOrderedValueMap() const {
        return std::map<Value2, T, LessThan>(LessThan(this));
    }
     ...
     */
    template <typename T>
    stdx::unordered_map<Value2, T, Hasher, EqualTo> makeUnorderedValueMap() const {
        return stdx::unordered_map<Value2, T, Hasher, EqualTo>(0, Hasher(this), EqualTo(this));
    }
...
using Value2Set = std::set<Value2, Value2Comparator::LessThan>;

using Value2UnorderedSet =
    stdx::unordered_set<Value2, Value2Comparator::Hasher, Value2Comparator::EqualTo>;

template <typename T>
using Value2Map = std::map<Value2, T, Value2Comparator::LessThan>;

template <typename T>
using Value2UnorderedMap =
    stdx::unordered_map<Value2, T, Value2Comparator::Hasher, Value2Comparator::EqualTo>;
